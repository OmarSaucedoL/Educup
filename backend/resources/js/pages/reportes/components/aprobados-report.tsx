...
export default function AprobadosReport({ postulantesAprobados = [], searchQuery, setSearchQuery, cup, distribucionAprobados = [] }: AprobadosReportProps) {
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-neutral-200 bg-neutral-50/50 text-left text-xs font-bold tracking-wider text-neutral-500 uppercase dark:border-neutral-800 dark:bg-neutral-950/20">
                                        ...
                                        <th className="p-4 text-center font-bold w-[180px]">Ingresados (1ª Opción)</th>
                                        <th className="p-4 text-center font-bold w-[180px]">Ingresados (2ª Opción)</th>
                                        <th className="p-4 text-center font-bold w-[180px]">Aprobados sin Cupo</th>
                                        <th className="p-4 text-center font-bold w-[150px]">Cupos Libres</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
                                    {distribucionAprobados.map((item) => (
                                        <tr key={item.carrera_nombre} className="hover:bg-neutral-50/40 dark:hover:bg-neutral-950/20 transition-colors">
                                            ...
                                            <td className="p-4 text-center font-bold">
                                                <span className={item.aprobado_sin_cupo > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-neutral-500 dark:text-neutral-400'}>
                                            ...
                                            <td className="p-4 text-center">
                                                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold ${
                                                    item.cupos_sobrantes > 0 
                                                        ? 'bg-emerald-100/70 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-450' 
                                                        : 'bg-rose-100/70 text-rose-700 dark:bg-rose-900/30 dark:text-rose-450'
                                                }`}>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-950/20 text-neutral-500 font-bold text-xs uppercase tracking-wider text-left">
                                ...
                                <th className="p-4 font-bold">Preferencia</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
                            {paginatedEstudiantes.length > 0 ? (
                                paginatedEstudiantes.map((student) => (
                                    <tr key={student.id} className="hover:bg-neutral-50/40 dark:hover:bg-neutral-950/20 transition-colors">
                                        ...
                                        <td className="p-4 font-bold text-neutral-800 dark:text-neutral-200 uppercase">
                                            {student.carrera_asignada || 'No Asignada'}
                                        </td>
                                        <td className="p-4">
